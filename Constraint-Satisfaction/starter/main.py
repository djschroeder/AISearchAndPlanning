...
class MRV(VarSelector):
    ...
    """
    def select_variable(self, grid):
        smallest_domain = grid.get_width() + 1
        var_tuple = None
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                domain_size = len(grid.get_cells()[i][j])
                if 1 < domain_size < smallest_domain:
...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        """
        methods = [
                self.remove_domain_column,
                self.remove_domain_row,
                self.remove_domain_unit
            ]
        while Q:
            var = Q.pop()
            row, col = var
            ...
            for method in methods:
                var_assigned, failure = method(grid, row, col)
                if failure:
                    return True
                if var_assigned:
                    Q.extend(var_assigned)
        return False
...
class Backtracking:
    ...
    def search(self, grid, var_selector):
        ...
        if not self.is_preprocessed:
            self.ac3.pre_process_consistency(grid)
        ...
        if var:
            ...
            for d in grid.get_cells()[row][col]:
                grid_copy = grid.copy()
                grid_copy.get_cells()[row][col] = d
                if self.ac3.consistency(grid_copy, [var]):
                    continue
...
def run_backtracking(problem, heuristic_class):
    g = Grid()
    g.read_file(problem)
    start = time.time()
    bt = Backtracking()
    solution = bt.search(g, heuristic_class())
    end = time.time()
    return end - start

# Function to be called by each process
def process_function(problem):
    time_fa = run_backtracking(problem, FirstAvailable)
    time_mrv = run_backtracking(problem, MRV)
    return time_fa, time_mrv
...
if __name__ == "__main__":
    ...
    with Pool(processes=multiprocessing.cpu_count()) as pool:
        results = pool.map(process_function, problems)

    # Unpack results
    run_time_fa, run_time_mrv = zip(*results)

    # Plot the results
    plotter = PlotResults()
    plotter.plot_results(run_time_mrv, run_time_fa, "MRV", "FA", "run_times.png")
...
for p in problems:
    ...
    run_time_fa.append(end - start)

    start = time.time()
    bt = Backtracking()
    solution = bt.search(g, MRV())
    end = time.time()
    run_time_mrv.append(end - start)

plotter = PlotResults()
plotter.plot_results(run_time_mrv, run_time_fa,
                     "MRV",
                     "FA", "run_times.png")
