...
def dijkstra(start, goal, map):
    ...
    while len(OPEN):
        ...
        if parent == goal:
            return parent.get_cost(), nodeCount
        for child in map.successors(parent):
            hash = child.state_hash()
            child.set_cost(child.get_g())
            if hash not in CLOSED:
                nodeCount += 1
                heapq.heappush(OPEN, child)
                CLOSED[hash] = child
            if hash in CLOSED and child < CLOSED[hash]:
                CLOSED[hash].set_cost(child.get_cost())
                CLOSED[hash].set_g(child.get_cost())
                heapq.heapify(OPEN)
    return -1, nodeCount
            

def A(start, goal, map):
    OPEN = [start]
    CLOSED = {start.state_hash(): start}
    nodeCount = 0
    while len(OPEN):
        ...
        for child in map.successors(parent):
            hash = child.state_hash()
            child.set_cost(child.get_g() + h(child, goal))
            if hash not in CLOSED:
                nodeCount += 1
                heapq.heappush(OPEN, child)
                CLOSED[hash] = child
            if hash in CLOSED and child < CLOSED[hash]:
                CLOSED[hash].set_cost(child.get_cost())
                CLOSED[hash].set_g(child.get_g())
                heapq.heapify(OPEN)
    return -1, nodeCount

def h(stateOne,stateTwo):
    dx = abs(stateOne.get_x() - stateTwo.get_x())
    dy = abs(stateOne.get_y() - stateTwo.get_y())
    minimum = min(dx, dy)
    return 1.5 * minimum + abs(dx - dy)