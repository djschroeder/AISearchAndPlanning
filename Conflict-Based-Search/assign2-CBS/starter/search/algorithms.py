...
class CBSState:
    ...
    def compute_cost(self):
        ...
        for agent in range(self._k):
            AStar_instance = AStar(self._map)
            cost, self._paths[agent] = AStar_instance.search(self._starts[agent], self._goals[agent], self._constraints[agent])
            if cost == -1:
                self._cost = -1
                break
            else:
                self._cost += cost
        return self._paths, self._cost
    ...
    def is_solution(self):
        ...
        states = set()
        for agent in range(self._k):
            for state in self._paths[agent]:
                state_cost = (state, state.get_g())
                if state_cost not in states:
                    states.add(state_cost)
                else:
                    return False, state_cost
        return True, None
    ...
    def successors(self):
        ...
        """
        successors = list()
        solution, state_conflict = self.is_solution()
        if not solution:
            for agent in range(self._k):
                for state in self._paths[agent]:
                    state_cost = (state, state.get_g())
                    if state_conflict == state_cost:
                        c = CBSState(self._map, self._starts, self._goals)
                        c._constraints = copy.deepcopy(self._constraints)
                        c.set_constraint(state_conflict[0], state_conflict[1], agent)
                        successors.append(c)
                        if len(successors) == 2:
                            return successors
